...
class ToolMetadata(BaseModel):
    ...
    Attributes:
        ...
        tags: Optional tags for advanced filtering
...
class BaseTool(ABC, Generic[TParams]):
    ...
    def _err(
        ...
        *,
        ...
        from toolcase.foundation.errors.types import _EMPTY_CONTEXTS, ErrorContext, _EMPTY_META
        ctx = (ErrorContext(operation=f"tool:{self.metadata.name}", location="", metadata=_EMPTY_META),)
        return Result(ErrorTrace(message=message, contexts=ctx, error_code=code.value, recoverable=recoverable, details=None), _ERR)
    ...
    def _err_from_exc(self, exc: Exception, context: str = "") -> ToolResult:
        ...
        return Result(ErrorTrace(message=msg, contexts=contexts, error_code=classify_exception(exc).value, recoverable=True, details=traceback.format_exc()), _ERR)
